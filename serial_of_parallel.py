...
x_split=np.array_split(points_x,12)

forLD = np.array([])
backLD = np.array([])
propLD = np.array([])
for i in range(len(x_split)):
    calc = xrangeLD(x_split[i])

    forLD = np.concatenate((forLD,np.array(calc[0])))
    backLD = np.concatenate((backLD,np.array(calc[1])))
    propLD = np.concatenate((propLD,np.array(calc[2])))
