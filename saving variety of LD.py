# -*- coding: utf-8 -*-
"""
Created on Tue Jan 18 11:21:50 2022
...
def LagrangianDesc(grid1,grid2, tau, p):
    ...
    for i in range(len(points_x)):
        for j in range(len(points_y)):
           ...
           #     funcptr = Hz0.address # address to ODE function for
                ...
                v = vect2(np.array(x),np.array(y),np.array(px),np.array(py))
                
        
                #cacluating the LD with p-norm, p =0.5
                intermedLD = np.sum(0.001*np.abs(v)**p, axis=1)
...
for k in range(len(p)):
        ...
        Ld = LagrangianDesc(200,200,5,p[k])
        
        np.savetxt("PropLDgridtau5grid200p"  + str(p[k]) +"back.txt", Ld )
